...
For ordinary Pi classifiers, `approvals.thinking` stores the requested reasoning level. The approval UI uses the selected Pi model's capability metadata and shows only the levels supported by that classifier; the effective value is clamped with Pi's normal rules when a saved level is unsupported by the current model. Jev uses its typed probability threshold path and does not use `approvals.thinking`.

When `/login openai-codex` makes an `openai-codex-responses` model template available, **Approvals → Auto model** also offers `openai-codex/codex-auto-review`. Fabric derives this classifier-only entry from that template, so it is absent when Codex is unavailable and does not appear in ordinary `/model`, agent, or actor model pickers. Its reasoning picker currently exposes **Low**, **Medium**, **High**, **XHigh**, and **Max**. A real registered model with that exact key takes precedence in both the approval picker and runtime.
